...
void Tracer::trace_exception(int64_t cause, uint32_t insn)
{
#ifdef DROMAJO
    ...
    if (trace_log_) {
        (*trace_log_) << fmt::format("EXC {} {:x}", cause, insn) << std::endl;
    }
}
...
#ifdef ITRACE
    room::Tracer::get_singleton().trace_exception(cause, insn);
#endif
}
