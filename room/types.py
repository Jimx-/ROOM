...
class MicroOp(Record):
    ...
    def __init__(self, params, name=None, src_loc_at=0):
        ...
        super().__init__([
            ...
            ('inst', 32),
            ('is_rvc', 1),
            ('iq_type', W(IssueQueueType)),
            ('fu_type', W(FUType)),
            ('br_type', W(BranchType)),
            ('opa_sel', W(OpA)),
            ('opb_sel', W(OpB)),
            ('imm_sel', W(ImmSel)),
            ('alu_fn', W(ALUOperator)),
            ('alu_dw', W(ALUWidth)),
            ('csr_addr', 12),
            ('csr_cmd', W(CSRCommand)),
            ('is_load', 1),
            ('is_sta', 1),
            ('is_std', 1),
            ('issue_uops', 2),
            ...
            ('is_jal', 1),
            ('is_jalr', 1),
            ('br_tag', range(max_br_count)),
            ('br_mask', max_br_count),
            ('ftq_idx', range(ftq_size)),
            ('edge_inst', 1),
            ('pc_lsb', range(fetch_bytes)),
            ('taken', 1),
            ('imm_packed', 20),
